...
| `--bbr_startup_loss_threshold_percent` | `0` | Process-wide (applies to every connection the process makes, not just one). BBRv1's startup phase gives up probing for more bandwidth -- settling for whatever it's already reached -- once a round sees enough packet-loss events *and* the lost bytes exceed this percentage of bytes in flight. QUICHE's real default is effectively `2` (2%). On a path with genuine baseline loss above that which can still sustain a much higher real bandwidth once ramped (e.g. a long-haul, moderately lossy link), this fires prematurely and the connection settles for far less than the path can actually do. `0` leaves QUICHE's real default unchanged; a value here is a percent (e.g. `50` for 50%), not a fraction. Pairs with `--bbr_startup_full_loss_count`. |
| `--bbr_startup_full_loss_count` | `0` | Process-wide. The other threshold paired with `--bbr_startup_loss_threshold_percent` -- minimum number of distinct loss-detection events (not lost packets) in one round before that percentage check even applies. QUICHE's real default is `8`. `0` leaves that default unchanged. |
