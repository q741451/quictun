...
#   asm/hwcap.h      abseil cpu_detect.cc on aarch64, for HWCAP_{CPUID,CRC32,PMULL}
#
...
UNISTD_EOF
  ...
  # abseil's CRC CPU detection includes this on aarch64+linux only, and reads
  # exactly three bits out of it via getauxval(AT_HWCAP). Values are from the
  # kernel's arch/arm64/include/uapi/asm/hwcap.h -- UAPI, so fixed.
  mkdir -p "$sysroot/include/asm"
  cat > "$sysroot/include/asm/hwcap.h" <<'HWCAP_EOF'
/* Generated by build/toolchain/setup_toolchain.sh -- see install_uapi_shims. */
#ifndef _QUICTUN_UAPI_SHIM_ASM_HWCAP_H
#define _QUICTUN_UAPI_SHIM_ASM_HWCAP_H
